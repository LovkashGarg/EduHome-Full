...
let res=await fetch("http://localhost:5000/api/v1/classes/createliveclasses",{
  ...
  <div className='flex justify-center items-center py-[100px] '>
    ...
    <form onSubmit={handlesubmit} >
        ...
        <div className='flex  flex-col w-[500px] rounded-[30px] gap-[20px] h-[400px] justify-center items-center border-[3px] border-green-500'>
        {ClassTeacher}
        <h2 className='text-[40px] font-mono '>Create Live Classes</h2>
        <label htmlFor="ClassTitle">Class Title :<input type="text" name='ClassTitle' placeholder='Carbon and Compounds'  onChange={(e)=>{
        setClassTitle(e.target.value);
        }}/></label>
        <label htmlFor="Timescheduled">Time scheduled :<input type="time" name='Timescheduled'  onChange={(e)=>{
        setClassTime(e.target.value);
        }}/></label>
        <label htmlFor="Classtype">
        Class Type:
            <select name="Paid_free"  onChange={(e)=>{
        ...
        <label htmlFor="ClassCategory">
            ...
            <button type="submit" className='bg-red-600 text-white w-[200px] h-[30px] rounded-[40px]' >Submit</button>
            </div>
    </form>
  </div>
  )
